...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(CoubViewProvider.viewType, provider));
    context.subscriptions.push(vscode.commands.registerCommand('coub-panel.next', () => {
        provider.nextCoub();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('coub-panel.back', () => {
        provider.previousCoub();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('coub-panel.refresh', () => {
        provider.refreshFeed();
    }));
}
class CoubViewProvider {
    _extensionUri;
    static viewType = 'coub-panel.view';
    _view;
    _coubQueue = [];
    _history = [];
    _historyIndex = -1;
